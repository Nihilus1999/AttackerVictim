...
import com.ucab.cmcapp.common.exceptions.mappers.FindExceptionMapper;
...
class FindExceptionMapperTest {
    @Test
    void testToResponse() {
        // Crear un objeto AuthenticationException con los valores esperados
        FindException exception = new FindException("Mensaje de error");
        ...
        try {
            ...
            when(registry.getProperty(Registry.EXC_FINDDB_CODE)).thenReturn("1");
            when(registry.getProperty(Registry.EXC_FINDDB_MSG)).thenReturn("Mensaje de error");

        } catch (NullPointerException e) {
            // Manejar la excepción NullPointerException

        } finally {
            FindExceptionMapper mapper = new FindExceptionMapper();
